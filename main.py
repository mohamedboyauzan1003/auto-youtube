...
W, H       = 1080, 1920
FPS        = 30
NUM_SCENES = 7          # 7 escenas = ~25-30s — sweet spot retencion Shorts
MIN_AUDIO_S        = 1.5
MIN_IMAGE_KB       = 15
...
class ColorLog(logging.Formatter):
    COLORS = {"DEBUG":"[37m","INFO":"[36m","WARNING":"[33m","ERROR":"[31m","SUCCESS":"[32m"}
    RESET = "[0m"
...
def update_upload_guard(video_id):
    """Registra el timestamp del ultimo upload exitoso."""
    try:
    ...
    for attempt in range(SCRIPT_RETRIES):
        try:
            ...
            if r.status_code == 429:
                wait = 15 * (2 ** attempt)
                log.warning(f"Rate limit — waiting {wait}s..."); time.sleep(wait); continue
            ...
            if start == -1 or end <= start:
                log.warning("No JSON found, retrying..."); continue
            script = json.loads(raw[start:end])
            if "scenes" not in script or len(script["scenes"]) < 5:
                log.warning(f"Only {len(script.get('scenes',[]))} scenes, retrying..."); continue
            ...
            log.error(f"JSON error attempt {attempt+1}: {e}"); time.sleep(5)
        except Exception as e:
            log.error(f"Groq attempt {attempt+1}: {e}"); time.sleep(5)
...
# ═══════════════════════════════════════════════════════════
ANIME_STYLES = [
    # Monster (Naoki Urasawa) — el anime de manipulacion/psicologia por excelencia
    "Naoki Urasawa Monster anime style, realistic dark psychological thriller art",
    "Monster anime Johan Liebert aesthetic, cold manipulative atmosphere",
    "Monster anime Naoki Urasawa art style, realistic faces deep shadows",
    # Estilos trend actuales de dark psychology content
    "dark psychological anime cinematic, hyper-realistic dramatic faces",
    "studio mappa dark anime, intense close-up facial expressions",
    "attack on titan dark cinematic anime, oppressive atmosphere",
    "demon slayer kimetsu no yaiba, dramatic lighting emotional intensity",
    "jujutsu kaisen dark aesthetic, surreal psychological imagery",
]

# Prompts de imagen que DEBEN incluir referencias a Monster cuando aplique
MONSTER_SCENE_INSERTS = [
    "Johan Liebert standing in shadows watching from distance, Monster anime style",
    "close-up of cold blue eyes with no emotion, Monster anime Johan style",
    "dark hospital corridor at night, Monster anime atmosphere",
    "man reading psychology book alone under single lamp, Monster anime realistic style",
    "figure manipulating others from shadows like Johan Liebert, Monster anime",
    "empty street at night rain reflection, Monster anime Urasawa style",
    "chess pieces on table one figure controlling all others, Monster anime style",
]
...
def validate_image(path):
    try:
        ...
        if p.stat().st_size < MIN_IMAGE_KB * 1024: return False, f"too small"
...
def generate_image(prompt, index):
    ...
    if cache_path.exists():
        ...
        if ok:
            ...
            shutil.copy(str(cache_path), out); log.info(f"Image {index+1} from cache"); return out

    style = random.choice(ANIME_STYLES)

    # 40% de probabilidad de usar una escena directa de Monster (Johan Liebert)
    # — es el personaje de manipulacion mas reconocido del anime, genera engagement
    if random.random() < 0.40:
        monster_insert = random.choice(MONSTER_SCENE_INSERTS)
        full_prompt = (
            f"{monster_insert}, {prompt}, "
            "masterpiece, best quality, ultra detailed, 8k, "
            "dramatic cinematic lighting, deep psychological atmosphere, "
            "realistic faces, cold color palette, professional digital art, "
        ...
        full_prompt = (
            ...
            "intense emotional close-up, trending dark aesthetic, "
    ...
    for attempt in range(IMAGE_RETRIES):
        ...
        if path:
            ...
            if ok:
                ...
                log.success(f"Image {index+1} OK ({model}, attempt {attempt+1})"); return path
            log.warning(f"Image {index+1} invalid ({reason})")
        wait = 4 * (attempt+1); log.warning(f"Image {index+1} retry in {wait}s..."); time.sleep(wait)
    ...
    if hf_token:
        for hf_model in HF_MODELS:
            ...
            if path:
                ...
                if ok:
                    import shutil; shutil.copy(path, str(cache_path))
                    log.success(f"Image {index+1} OK (HuggingFace)"); return path
            time.sleep(3)

    log.error(f"Image {index+1} all failed — gradient"); return _dark_gradient_fallback(index)
    ...
    try:
        r = requests.post(f"https://api-inference.huggingface.co/models/{model}",
            ...
            json={"inputs": prompt, "parameters": {
                "negative_prompt": "lowres, bad anatomy, blurry, watermark, text",
                "width": 832, "height": 1216, "num_inference_steps": 28, "guidance_scale": 7.0},
...
# ═══════════════════════════════════════════════════════════
#  TEXT RENDERER — estilo TikTok/CapCut palabra a palabra
# ═══════════════════════════════════════════════════════════
def render_text_frame(base_arr, text, word_progress, frame_idx, scene_num, total_scenes):
    ...
    for y in range(int(h*0.42)): gd.line([(0,y),(w,y)], fill=(0,0,0,int(190*(1-y/(h*0.42))**1.15)))
    img = Image.alpha_composite(img, gt)

    # Gradiente inferior
    gb = Image.new("RGBA",(w,h),(0,0,0,0)); gd2 = ImageDraw.Draw(gb); bh = int(h*0.18)
    for y in range(h-bh,h): gd2.line([(0,y),(w,y)], fill=(0,0,0,int(215*((y-(h-bh))/bh)**1.0)))
...
def synth_one(text, index):
    ...
    if Path(cache_path).exists() and Path(cache_path).stat().st_size > min_bytes:
        ...
        if ok:
            ...
            log.info(f"Audio {index+1} from cache ({dur:.1f}s)"); return out_path
    for attempt in range(TTS_RETRIES):
        ...
        try:
            ...
            if not p.exists() or p.stat().st_size < min_bytes:
                log.warning(f"Audio {index+1} too small, retry {attempt+1}..."); time.sleep(2+attempt); continue
            ...
            if attempt == 1: log.warning(f"Switching to fallback voice")
        time.sleep(2+attempt)
    log.error(f"Audio {index+1} failed — silence"); _make_silence(out_path, 3.0); return out_path
...
# ═══════════════════════════════════════════════════════════
#  MUSICA — Freesound (real) + sintetica (fallback)
# ═══════════════════════════════════════════════════════════
...
def try_freesound_music(duration, mood):
    ...
    if not api_key:
        log.warning("FREESOUND_API_KEY not set — synthetic fallback"); return None
    ...
    log.warning("Freesound: no track found — synthetic fallback"); return None
...
def generate_music(duration=60, mood=None):
    ...
    try:
        ...
        log.success(f"Synthetic music OK ({duration}s, mood={mood})"); return wav
    except Exception as e: log.error(f"Music error: {e}"); return None
...
def build_scene(base_img, audio_path, text, scene_idx, total_scenes):
    ...
    reveal_frames = int(FPS*1.3)  # mas rapido = mas dinamico
...
def validate_video(path, title, description, tags):
    ...
    except Exception as e: errors.append(f"Cannot open: {e}")
    ...
    if errors:
        ...
        return False
    log.success("Checklist passed"); return True
    ...
    if music_path and Path(music_path).exists():
        try:
            ...
            if float(probe.stdout.strip() or 0) < 1.0:
                log.warning("Music invalid, regenerating..."); music_path = generate_music(int(final_duration)+8); music_source = "synthetic"
            ...
            if result.returncode == 0 and Path(output).stat().st_size > 500_000:
                ...
                log.warning(f"ffmpeg mix failed: {result.stderr[-300:]}"); import shutil; shutil.copy(voice_only, output)
        except Exception as e:
            log.warning(f"Music mix error: {e}"); import shutil; shutil.copy(voice_only, output)
    else:
        log.warning("No music — voice only"); import shutil; shutil.copy(voice_only, output)
    return output
    ...
    try:
        ...
        for attempt in range(UPLOAD_RETRIES):
            try:
                ...
                vid = response["id"]
                log.success(f"UPLOADED: https://www.youtube.com/watch?v={vid}"); return vid
            except Exception as e:
...
def log_video_metadata(video_id, script, duration):
    ...
    try:
        with open(csv_path,"a",newline="",encoding="utf-8") as f:
            w = csv.writer(f)
            if is_new: w.writerow(["date","video_id","url","title","topic","first_scene_text","voice","rate","pitch","subtitle_theme","music_mood","duration_s","num_scenes"])
            w.writerow([time.strftime("%Y-%m-%d %H:%M"), video_id or "FAILED",
                        f"https://www.youtube.com/watch?v={video_id}" if video_id else "",
                        script["title"], script.get("_topic",""), script["scenes"][0]["text"] if script.get("scenes") else "",
                        VOICE, VOICE_RATE, VOICE_PITCH, THEME["name"], _chosen_mood, f"{duration:.1f}", len(script.get("scenes",[]))])
        log.success(f"Metadata logged")
    except Exception as e: log.warning(f"Metadata error: {e}")
...
if __name__ == "__main__":
    ...
    # UPLOAD GUARD: comprobar si ya se publico en la ultima hora
    if not check_upload_guard():
        log.info("Upload guard active — exiting cleanly (no duplicate upload)")
        exit(0)
